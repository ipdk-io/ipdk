...
class SocketTestEnvironment(unittest.TestCase):
    def setUp(self):
        ...
        self.fake_socket = socket.socket(
...
class HotPlugValidationBaseTestCases:
    ...
    class Tests(SocketTestEnvironment):
        def setUp(self):
            ...
            self.provider = HotPlugProvider(self.shared_dir_path,
                                            self.host_shared_dir_path)
        ...
        def test_pass_valid_sockets(self, mock_subprocess_call):
            ...
            try:
                self.disk_operation(
                    self.fake_socket_path, self.fake_socket_path)
            except:
        ...
        def test_hot_plug_generates_correct_device_id(self,
        ...
        def test_pass_none_vm(self, unused):
            with self.assertRaises(ValueError) as ex:
                self.disk_operation(
                    None, self.fake_socket_path)

        def test_pass_none_vhost(self, unused):
            with self.assertRaises(ValueError) as ex:
                self.disk_operation(
                    self.fake_socket_path, None)
...
class HotPlugValidation(HotPlugValidationBaseTestCases.Tests):
    ...
    def test_hot_plug_call_args(self, mock_device_id_generator,
                                mock_subprocess_call):
        device_id = "42"
        mock_subprocess_call.return_value = 0
        ...
        mock_subprocess_call.assert_called_once_with(
            ...
            device_id, shell=True)

    @patch("hot_plug_provider.subprocess.call")
    @patch.object(HotPlugProvider, "_generate_device_id")
    def test_hot_plug_call_args_with_trailing_whitespaces(self,
                                                          mock_device_id_generator,
                                                          mock_subprocess_call):
        device_id = "42"
        mock_subprocess_call.return_value = 0
        mock_device_id_generator.return_value = device_id
...
class HotUnplugValidation(HotPlugValidationBaseTestCases.Tests):
    ...
    def test_hot_plug_call_args(self, mock_device_id_generator,
        ...
        self.disk_operation(vm, vhost)
        mock_subprocess_call.assert_called_once_with(
            "/hot-unplug.sh " + os.path.join(self.shared_dir_path, vm) + " " +
            device_id, shell=True)
